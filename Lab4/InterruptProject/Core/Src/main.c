...
char USART_NoDelayReadChar(void);
void USART_LEDControl(void);

int USART3NewVal = 0;
int USART3BufferVal = 0;
char USART3InputStr[2];
/* USER CODE BEGIN PFP */
...
int main(void)
{
  ...
  while (1)
  {
		...
		{
				//RED
				if(USART3InputStr[0] == 'r')
				{
					if(USART3InputStr[1] == '0')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_RESET);
						USART_StringChar("RED LED Disabled");
					}
					else if(USART3InputStr[1] == '1')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);
						USART_StringChar("RED LED Enabled");
					}
					else if(USART3InputStr[1] == '2')
					{
						HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_6);
						USART_StringChar("RED LED Toggled");
					}
					else USART_StringChar("Invalid input");
				}
				//ORANGE
				else if(USART3InputStr[0] == 'o')
				{
					if(USART3InputStr[1] == '0')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_RESET);
						USART_StringChar("ORANGE LED Disabled");
					}
					else if(USART3InputStr[1] == '1')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_SET);
						USART_StringChar("ORANGE LED Enabled");
					}
					else if(USART3InputStr[1] == '2')
					{
						HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_8);
						USART_StringChar("ORANGE LED Toggled");
					}
					else USART_StringChar("Invalid input");
				}
				//GREEN
				else if(USART3InputStr[0] == 'g')
				{
					if(USART3InputStr[1] == '0')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_RESET);
						USART_StringChar("GREEN LED Disabled");
					}
					else if(USART3InputStr[1] == '1')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET);
						USART_StringChar("GREEN LED Enabled");
					}
					else if(USART3InputStr[1] == '2')
					{
						HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_9);
						USART_StringChar("GREEN LED Toggled");
					}
					else USART_StringChar("Invalid input");
				}
				//BLUE
				else if(USART3InputStr[0] == 'b')
				{
					if(USART3InputStr[1] == '0')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_RESET);
						USART_StringChar("BLUE LED Disabled");
					}
					else if(USART3InputStr[1] == '1')
					{
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_SET);
						USART_StringChar("BLUE LED Enabled");
					}
					else if(USART3InputStr[1] == '2')
					{
						HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_7);
						USART_StringChar("BLUE LED Toggled");
					}
					else USART_StringChar("Invalid input");
				}
				else
				{
					USART_StringChar("Invalid input");
				}
				
				USART_StringChar(" Input User Command: ");
				USART3NewVal = 0;
			}
		}
}
...
/**
  * @brief  Handles the interrupts for the USART3 peripheral.
  * @retval None
  */
void USART3_4_IRQHandler(void)
{
	if(USART3BufferVal == 0)
	{
		USART3InputStr[0] = USART_NoDelayReadChar();
		USART3BufferVal++;
	} else {
		USART3InputStr[1] = USART_NoDelayReadChar();
		USART3NewVal = 1;
		USART3BufferVal = 0;
	}
}
...
void USART_Init(void)
{
	...
	//Enable the recieve interrupt in NVIC
  NVIC_EnableIRQ(USART3_4_IRQn);
	
	//Set the priority of the recieve interrupt in the NVIC
...
/**
  * @brief  Reads the UART read reg value.
  * @retval Value that was read from the USART.
  */
char USART_NoDelayReadChar(void)
{
...
void USART_LEDControl(void)
{
	...
	switch(input)
	{
		...
		default:
		{
			char str[] = "Invalid Character Inputted!";
