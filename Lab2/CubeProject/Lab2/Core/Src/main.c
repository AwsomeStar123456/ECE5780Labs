...
int main(void)
{
  ...
  SystemClock_Config();
	...
	//Set up GPIO pins 6, 7, 8, and 9 to drive the LEDS on the STM.
	GPIO_InitTypeDef initC = {GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9,
	GPIO_MODE_OUTPUT_PP, GPIO_SPEED_FREQ_LOW, GPIO_NOPULL};
	
	//Set the GPIO pin 0 for the user button.
	GPIO_InitTypeDef initA = {GPIO_PIN_0, GPIO_MODE_OUTPUT_PP, GPIO_SPEED_FREQ_LOW, GPIO_PULLDOWN};
	
	//Write out GPIO typedefs to the respective GPIOs.
	...
	//Set up the EXTI. Enable interrupt on zero and trigger on rising edge.
